...
def process_image(image):
    ...
    final_out = util.draw_lanes(warped, undist, lane_finder.ploty, lane_finder.left_line.best_fitx,
            lane_finder.right_line.best_fitx, lane_finder.y_eval, lane_finder.xm_per_pix, lane_finder.ym_per_pix, persp_transformer.Minv)
    util.put_text(final_out, lane_finder.left_line.radius_of_curvature, lane_finder.left_line.line_base_pos, 
            lane_finder.right_line.radius_of_curvature, lane_finder.right_line.line_base_pos)
    return final_out

# Generate video
lane_finder = lf.Lane_Finder(sample.shape)
project_clip = VideoFileClip(src_path + '/challenge_video.mp4')
project_clip_processed = project_clip.fl_image(process_image)
project_clip_processed.write_videofile(src_path + '/challenge_video_output.mp4', audio=False)